...
Item {
    ...
    readonly property bool inputsNeedProcessing: status.moveForward | status.moveBack
                                                 ...
                                                 | status.moveUp | status.moveDown | status.useMouse
    ...
    QtObject {
        ...
        function updatePosition(vector, speed, position) {
            ...
            var velocity = Qt.vector3d(direction.x * speed,
                                       direction.y * speed, direction.z * speed)
        ...
        function processInput() {
            ...
            if (moveForward)
                updatePosition(controlledObject.forward, forwardSpeed,
                               controlledObject.position)
            else if (moveBack)
                updatePosition(negate(controlledObject.forward), backSpeed,
                               controlledObject.position)

            if (moveRight)
                updatePosition(controlledObject.right, rightSpeed,
                               controlledObject.position)
            else if (moveLeft)
                updatePosition(negate(controlledObject.right), leftSpeed,
                               controlledObject.position)

            if (moveDown)
                updatePosition(negate(controlledObject.up), downSpeed,
                               controlledObject.position)
            else if (moveUp)
                updatePosition(controlledObject.up, upSpeed,
                               controlledObject.position)
